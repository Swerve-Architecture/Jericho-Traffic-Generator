...
// to get correct struct definitions
// from tcphdr, udphdr, etc.
#define __FAVOR_BSD
...
static void compute_udp_checksum(struct iphdr *pIph, unsigned short *ipPayload) {
    ...
    unsigned short udpLen = htons(udphdrp->uh_ulen);
    ...
    //printf("one's complementn");
      sum = ~sum;
    //set computation result
    udphdrp->uh_sum = ((unsigned short)sum == 0x0000)?0xFFFF:(unsigned short)sum;
}
